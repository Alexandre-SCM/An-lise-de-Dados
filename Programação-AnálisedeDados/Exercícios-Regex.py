...
"""
# RESOLUCAO
filtro = df["nome_aluno"] == "ALEXANDRE DE SOUSA CAMPOS MENEZES"
df.loc[filtro, :]
...
"""
# RESOLUCAO
filtro = df["nome_aluno"].str.contains("^M", case=False)
df.loc[filtro, :]
...
# r"^[ABC]"
# RESOLUCAO
filtro = df["nome_aluno"].str.contains(r"^[ABC]", case=False)
df.loc[filtro, :]
...
# r"^[A-Za-z]+ [A-Za-z]+$"
# RESOLUCAO
filtro = df["nome_aluno"].str.contains(r"^[A-Za]+ [A-Za]+$", case=False)
df.loc[filtro, :]
...
"""
# RESOLUCAO
filtre = df["nome_aluno"].str.contains(r"^[A-Za]+ [A-Za]+ [A-Za]+$", case=False)
df.loc[filtre, :]
...
# r"^[A-Za-z ]+$"
# RESOLUCAO
filtro = df["nome_aluno"].str.contains(r"^[A-Za-z ]+$", case=False)
df.loc[filtro, :]
...
"""
# RESOLUCAO
filtro = df["nome_aluno"].str.contains(r"^[A-Za]+ [A-Za]+ (SOUZA|LIMA|ROCHA)$", case=False)
df.loc[filtro, :]
...
# RESOLUCAO: complete aqui
filtro = df_clientes["email"].str.contains(r"@(gmail|outlook)\.com$", case=False)
df_clientes.loc[filtro, :]
...
# RESOLUCAO: complete aqui
filtro_a = df_leads["origem"].str.contains("instagram", case=False, na=False)
filtro_b = df_leads["campanha"].str.contains(r"^[A-Za-z_ ]+$", case=False, na=False)
df_leads.loc[filtro_a, :]